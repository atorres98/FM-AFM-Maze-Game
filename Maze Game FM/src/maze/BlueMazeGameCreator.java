...
public class BlueMazeGameCreator {
...
//	static HashMap<Integer, Room> roomKeys = new HashMap<Integer, Room>();
	...
	static Maze BluemadeMaze = new Maze();
	
	static HashMap<Integer, String[]> roomInfo = new HashMap<Integer, String[]>();
	
	static HashMap<Integer, LightBlueRoom> rooms = new HashMap<Integer, LightBlueRoom>();

	static HashMap<String, String[]> doorInfo = new HashMap<String, String[]>();
	
	static HashMap<String, BrownDoor> doors = new HashMap<String, BrownDoor>();
...
//					System.out.println("Sides of Room in order of N,S,E,W: " + Arrays.toString(roomSides));
	...
	public static void createRoom() {
		...
		for (int count: roomInfo.keySet()) {
			...
			LightBlueRoom newRoom = new LightBlueRoom(count);
			System.out.println("new room-- " + newRoom);
			System.out.println("Key set:" + roomInfo.keySet().toString());
			BluemadeMaze.addRoom(newRoom);
	...
	public static void createDoor() {
		...
		for (String door : doorInfo.keySet()) {
			...
			LightBlueRoom roomL = rooms.get(roomLID);
			LightBlueRoom roomR = rooms.get(roomRID);
			BrownDoor  newDoor = new BrownDoor(roomL, roomR);
...
//			doors.put(line[1], newDoor);
	...
	public static void setSide() {
		...
		for (int roomNum: roomInfo.keySet()) {
			...
			LightBlueRoom currentRoom = rooms.get(roomNum);
			...
			for (String side: sideInfo) {
				...
				if (side.equals("wall")) {
					...
					currentRoom.setSide(currentDirection, new BlueWall());
				} else if (side.startsWith("d")) {
					BrownDoor currentDoor = doors.get(side);
					...
					LightBlueRoom sideRoom = rooms.get(sideRoomNum);
	...
	public static Maze createMaze(String path) {
		...
		System.out.println("current room: " + BluemadeMaze.getCurrentRoom());
		System.out.println("madeMaze " + BluemadeMaze);
		return BluemadeMaze;
	}
