...
public class RedMazeGameCreator {
...
//	static HashMap<Integer, Room> roomKeys = new HashMap<Integer, Room>();
	...
	static Maze RedmadeMaze = new Maze();
	
	static HashMap<Integer, String[]> roomInfo = new HashMap<Integer, String[]>();
	
	static HashMap<Integer, Pink_Room> rooms = new HashMap<Integer, Pink_Room>();

	static HashMap<String, String[]> doorInfo = new HashMap<String, String[]>();
	
	static HashMap<String, Green_Door> doors = new HashMap<String, Green_Door>();
...
//					System.out.println("Sides of Room in order of N,S,E,W: " + Arrays.toString(roomSides));
	...
	public static void createDoor() {
		...
		for (String door : doorInfo.keySet()) {
			...
			Pink_Room roomL = rooms.get(roomLID);
			Pink_Room roomR = rooms.get(roomRID);
			Green_Door  newDoor = new Green_Door(roomL, roomR);
...
//			doors.put(line[1], newDoor);
	...
	public static void setSide() {
		...
		for (int roomNum: roomInfo.keySet()) {
			...
			Pink_Room currentRoom = rooms.get(roomNum);
			...
			for (String side: sideInfo) {
				...
				if (side.equals("wall")) {
					...
					currentRoom.setSide(currentDirection, new Red_Wall());
				} else if (side.startsWith("d")) {
					Green_Door currentDoor = doors.get(side);
					...
					Pink_Room sideRoom = rooms.get(sideRoomNum);
	...
	public static Maze createMaze(String path) {
		...
		System.out.println("current room: " + RedmadeMaze.getCurrentRoom());
		System.out.println("madeMaze " + RedmadeMaze);
		return RedmadeMaze;
	}
