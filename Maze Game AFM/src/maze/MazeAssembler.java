...
//	static HashMap<Integer, Room> roomKeys = new HashMap<Integer, Room>();
	...
	public static MazeFactory myfactory;
...
//					System.out.println("Sides of Room in order of N,S,E,W: " + Arrays.toString(roomSides));
	...
	public static void createRooms() {
		...
		System.err.println("my factory is " + myfactory);
		for (int count: roomInfo.keySet()) {
			Room newRoom = myfactory.makeRoom(count);
	...
	public static void createDoors() {
		...
		for (String door : doorInfo.keySet()) {
			...
			Door  newDoor = myfactory.makeDoor(roomL, roomR);
...
//			doors.put(line[1], newDoor);
		...
		for (int roomNum: roomInfo.keySet()) {
			...
			for (String side: sideInfo) {
				...
				if (side.equals("wall")) {
					...
					currentRoom.setSide(currentDirection, myfactory.makeWall());
	...
	public static Maze createMaze(String path, MazeFactory factory) {
		myfactory = factory;
		parse(path);
		createRooms();
		createDoors();
